    ...
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/90 backdrop-blur-md">
      <nav className="container-abc flex items-center justify-between py-4">
        <Link href="/" className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.18em] text-blue-500">ABC</p>
        ...
        <div className="hidden items-center gap-6 md:flex">
          {links.map((link) => (
            <Link key={link.href} href={link.href} className="text-sm font-semibold text-slate-700 hover:text-blue-500">
              {link.label}
            </Link>
          ))}
        </div>
