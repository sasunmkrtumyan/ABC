    ...
    <main className="container-abc py-12">
      <div className="rounded-2xl bg-white p-8 shadow-sm">
        <div className="flex flex-col gap-6 md:flex-row md:items-center">
          {partner.logoUrl ? (
            <Image src={partner.logoUrl} alt={partner.name?.en || "logo"} width={110} height={110} className="rounded-xl object-cover" />
          ) : (
            <div className="h-24 w-24 rounded-xl bg-slate-100" />
          )}

          <div>
            <h1 className="text-4xl font-black text-brand.dark">{partner.name?.[language] || partner.name?.am}</h1>
            <p className="mt-3 text-slate-600">{(partner.tags || []).join(", ")}</p>
            <p className="mt-2 text-slate-600">{partner.email}</p>
          </div>
        </div>

        <p className="mt-8 whitespace-pre-line text-slate-700">{partner.description?.[language] || partner.description?.am}</p>
